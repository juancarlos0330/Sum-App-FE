...
import { getData, getDetailData } from "./../actions/projectActions";
...
const List = (props) => {
  ...
  useEffect(() => {
    // if (props.verifycode !== "") {
    props.getData({ code: props.verifycodes });
    // }
  }, []);
  ...
  const getAryfunc = async (data) => {
    ...
    isEmpty(data)
      ...
      : await data.map(async (items) => {
          ...
          // await items.datas.map((item, key) => {
          //   allary.push({
          //     pid: items._id,
          //     id: item._id,
          //     name: items.filename + " " + item.project,
          //     status: 2,
          //   });
          // });
        });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.title}>
        <Text style={styles.titletext}>Your projects</Text>
      </View>
      <View>
        {isEmpty(result) ? (
          <View>
            <Text style={{ textAlign: "center" }}>Project Not Found</Text>
          </View>
        ) : (
          <View>
                  ...
                  {item.status !== 2 ? (
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        props.setpageflagfunc("detailproject");
                      }}
                    >
                      <FontAwesome name="edit" size={40} color="#fff" />
                    </TouchableOpacity>
...
export default connect(mapStateToProps, { getData, getDetailData })(List);
